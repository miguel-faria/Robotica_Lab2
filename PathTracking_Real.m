...
sonar_alert_dist_level_1 = 500; %distance in mm, warning level one used to prevent the robot from turning in that direction
...
[path,x_res,y_res] = Create_Path(mapa_bw, path_points, mode);
if isempty(path_points)
    ...
    [path_real,x_res,y_res] = Create_Path(mapa_bw, path_points_real, mode);
end
...
%Timer Set
timer_obj = timer('TimerFcn', 'myTimeCallback(sp, v_real, w_real, x, y, map, velocities, path)', 'Period', 10, 'ExecutionMode', 'fixedSpacing');
start(timer_obj);
...
while point_index ~= path_real_len
    ...
    if point_index < half_path_len
        path_interval = path_real(1:half_path_len,:);
        point_index = Find_Nearest_Point(x, y, path_interval);
    else
        path_interval = path_real(half_path_len:path_len,:);
    ...
    k2 = b*abs(speed)*robot_internal_error*scale_error;
    ...
    if(optical_data(2) <= sonar_alert_dist_level_2)
        ...
        adjust_optical = adjust_vision - (135 - abs(sensors_angles(2)))*(sonar_alert_dist_level_1 - optical_data(2));
    ...
    adjust_optical = adjust_optical / 4000*K*10;
    
    w_real = w_ref + adjust_robot_errors + adjust_optical;
    ...
    if abs(w_real) > 45
        w_real = 45 * sign(w_real);
    end
    
    if danger_level == 2
        v_real = fix(v_init/10);
    ...
    if point_index == 1
        ...
        delta_theta_map = theta_ref_map - atan2((y_ref_mpa - path(point_index-1,2)), (x_ref_map - path(point_index-1,1)));
    end
    ...
    theta_map = theta_map + w*delta_time + 0.001*random_val(3);
    
    x = x + 2*cos(theta_map)*2 + noise*2/50*random_val(1);
    y = y + 2*sin(theta_map)*2 + noise*random_val(2);
